...
public class PluginFile implements Closeable, AutoCloseable {
    private final JarFile jar;
    private final File dir;
    private boolean closed = false;

    public PluginFile(@NotNull File file) throws IOException {
    ...
    @Nullable
    public PluginFileEntry getEntry(@NotNull String name) {
        JarEntry entry = getJarEntry(name);
        if (jar != null && entry != null) {
            try {
                return new PluginFileEntry(jar.getInputStream(entry), entry.getCodeSigners());
            } catch (IOException ignore) {}
        }
        try {
            return new PluginFileEntry(Objects.requireNonNull(getResourceAsStream(name)), null);
